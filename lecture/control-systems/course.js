(function(){
  ...
  function loadTeachingLayer(done){
    if(!document.querySelector('link[href*="teaching.css"]')){
      const l=document.createElement('link'); l.rel='stylesheet'; l.href='./teaching.css?v=2'; document.head.appendChild(l);
    }
    loadScript('./teaching.js?v=1',()=>!!window.teachingEnhancements,()=>{
      loadScript('./practice.js?v=1',()=>!!window.practiceEnhancements,done);
    });
  }
  ...
  function render(){
    ...
    const practice = window.practiceEnhancements || {worked:{},labs:{}};
    ...
    const sections = unit.sections.map((s,i)=>{
      ...
      const workedExample=practice.worked&&practice.worked[key]?practice.worked[key]:'';
      return `<section class="cs-textbook-section"><h2 id="${sid}"><span class="cs-section-number">${num}</span>${s.title}</h2>${bridge}${visual}${steps}${s.html}${workedExample}</section>`;
    }).join('');
    ...
    const lectureLab=practice.labs&&practice.labs[id]?`<div id="lab">${practice.labs[id]}</div>`:'';
    ...
    article.innerHTML=`<p class="cs-eyebrow">${lectureLabel} · ${unit.sections.length} sections</p><h1>${unit.title}</h1><p class="cs-question">${unit.question}</p>${beginnerIntro}<div class="cs-goals"><strong>学完这一讲，你应该能：</strong><ul>${unit.goals.map(g=>`<li>${g}</li>`).join('')}</ul></div><div class="cs-lesson-map"><div class="cs-map-head"><span>Section map</span><strong>按小节学习，不要一次扫完整页</strong></div><div class="cs-map-grid">${sectionCards}</div></div>${sections}${lectureLab}<section class="cs-textbook-section cs-exercise-section"><h2 id="practice"><span class="cs-section-number">✓</span>本讲练习 <span class="cs-practice-badge">先做 worked example，再自己做</span></h2><div class="cs-practice">${unit.practice}</div></section><div class="cs-source-note"><strong>对应 ETH 材料</strong><span>${unit.source}</span><small>本站正文与 worked examples 均为独立重写；不公开或镜像 Moodle 原始 PDF / solutions。</small></div>${pager}`;

    toc.innerHTML=`<div class="cs-toc-label">On this lecture</div><a href="#top" class="cs-toc-top">${unit.id==='00'?'L00':'L'+unit.id} · ${unit.title}</a>${unit.sections.map((s,i)=>`<a href="#s${unit.id}-${i+1}">${unit.id==='00'?'0':Number(unit.id)}.${i+1} ${s.title}</a>`).join('')}${lectureLab?'<a href="#lab">⚙ Python lab</a>':''}<a href="#practice">✓ 本讲练习</a>`;

    article.id='unitArticle'; article.setAttribute('tabindex','-1'); layout.id='top';
    if(window.renderMathInElement){
      renderMathInElement(article,{delimiters:[{left:'$$',right:'$$',display:true},{left:'$',right:'$',display:false}],throwOnError:false,ignoredTags:['script','noscript','style','textarea','pre','code']});
    }
    const links=[...toc.querySelectorAll('a[href^="#"]')];
    const targets=links.map(a=>document.querySelector(a.getAttribute('href'))).filter(Boolean);
    if('IntersectionObserver' in window&&targets.length){
      const obs=new IntersectionObserver(entries=>{entries.forEach(entry=>{if(entry.isIntersecting){links.forEach(a=>a.classList.remove('is-active'));const hit=links.find(a=>a.getAttribute('href')==='#'+entry.target.id);if(hit)hit.classList.add('is-active');}});},{rootMargin:'-20% 0px -70% 0px'});
