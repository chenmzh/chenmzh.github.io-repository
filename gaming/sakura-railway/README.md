...
- 铁路改为**闭合环线**（约 924 m）：道口直道 → 田园小站 → 远端大弯 → 西侧农庄回程 → 村后，
  地图版为 3 节编组列车并永远运行；电影版保持原开放曲线与单节构图不变。
- 地图专属场景增加乡村站台、候车棚、站房、稻田秧苗、远端农舍、农具仓库、水塔和铁路维护设施，
  全部合批/实例化，只增加少量 draw call。
...
- 同一个 `createScene()`：`mapMode` 切换自由相机（SandboxCamera）、环形铁路、地图专属内容、
