...
项目只截取公开预览的第一段古筝拨弦作为 A3（约 220 Hz）参考音，再通过确定性的重采样变调覆盖旋律音域；同时叠加很轻的合成指甲瞬态、持续和声、8 音琶音与 Voice Pool。最终 WAV/MP3 是新的混合作品，不是把原始预览直接作为整曲播放。

如果上游预览链接未来失效，项目仍保留 `music/source/create_guzheng.py --engine additive` 的无采样回退路径；但要复现当前 v1.3 的真实录音质感，需要保留项目内的 WAV 参考文件。
